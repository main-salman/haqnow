...
1. ✅ Always use `.kubeconfig` (with leading dot) - it's gitignored
2. ✅ Never commit files with `kubeconfig` in the name
3. ✅ Store sensitive values in `.env` (already gitignored)
4. ✅ Use Terraform outputs to reference paths, not hardcoded values
...
3. **If committed** - Follow the "If Kubeconfig is Accidentally Committed" steps above
